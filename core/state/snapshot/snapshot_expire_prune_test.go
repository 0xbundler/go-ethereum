...
func TestShrinkExpiredSubTree(t *testing.T) {
	db := memorydb.New()
	rawdb.WriteStorageSnapshot(db, accountHash, storageHash1, encodeSnapVal(NewRawValue([]byte("val1"))))
	rawdb.WriteStorageSnapshot(db, accountHash, storageHash2, encodeSnapVal(NewRawValue([]byte("val2"))))

	err := ShrinkExpiredSubTree(db, accountHash, storageShrink1, types.StateEpoch0, storageNodeHash1)
	assert.NoError(t, err)

	assert.Nil(t, rawdb.ReadStorageSnapshot(db, accountHash, storageHash1))
	assert.Nil(t, rawdb.ReadStorageSnapshot(db, accountHash, storageHash2))
	assert.Equal(t, encodeSnapVal(NewShrinkNodeWithEpoch(types.StateEpoch0, storageNodeHash1)), rawdb.ReadStorageSnapshot(db, accountHash, common.BytesToHashLeft(storageShrink1)))
}
...
func TestShrinkExpiredLeaf(t *testing.T) {
	...
	assert.Equal(t, encodeSnapVal(NewValueWithEpoch(types.StateEpoch0, common.Hash{})), rawdb.ReadStorageSnapshot(db, accountHash, storageHash1))
}
